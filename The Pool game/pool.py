...
def find_intersection(p0, p1, p2, p3):
    ...
    if (s_numer > denom) == denom_is_positive or (t_numer > denom) \
                         == denom_is_positive:
        return None  # no collision

    # collision detected
    t = float(t_numer) / denom
    intersection_point = [int(round(p0[0] + (t * s10_x))),
                          int(round(p0[1] + (t * s10_y)))]
...
def play(img, file_number):
    ...
    circles = cv2.HoughCircles(img, cv2.cv.CV_HOUGH_GRADIENT, 1, 20, param1=40,
                               param2=35, minRadius=20, maxRadius=100)
...
if __name__ == "__main__":
    ...
    img = cv2.imread('SampleImages/1.jpg')
